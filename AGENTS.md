...
## Architecture
- **`docker-compose.yml`** — production baseline. Uses `image: odoo:19`, mounts config/addons read-only.
...
- **`config/odoo.conf`**: `addons_path = /mnt/extra-addons`, `workers = 2`, `proxy_mode = True`. Admin password hash is PBKDF2 of `"admin"` — needed for DB manager at `/web/database/manager`.
